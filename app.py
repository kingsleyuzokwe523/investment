...
class DualDatabaseCollection:
    def __init__(self, collections, name):
        ...
        self.name = name
        self._sort_fields = None  # Store sort criteria
    ...
    def sort(self, *args, **kwargs):
        """
        Sort method that works like PyMongo's sort.
        Accepts either:
        - sort([('field', direction)])
        - sort('field', direction)
        """
        if len(args) == 1 and isinstance(args[0], list):
            # Called as sort([('field', direction)])
            self._sort_fields = args[0]
        elif len(args) == 2:
            # Called as sort('field', direction)
            self._sort_fields = [(args[0], args[1])]
        else:
            ...
            try:
                if collection is not None:
                    ...
                    if self._sort_fields is not None:
                        cursor = cursor.sort(self._sort_fields)
...
ALLOWED_ORIGINS = [
    ...
    "https://veloxtrades.com.ng",           # ← Your main domain
    "https://www.veloxtrades.com.ng",       # ← With www
    "https://velox-wnn4.onrender.com", 
    "https://investment-gto3.onrender.com",
    # Add your specific frontend URL if different
    "https://veloxtrades.com.ng:3000",      # If using port 3000
]
...
def add_cors_headers(response):
    ...
    if 'veloxtrades.com.ng' in origin or 'onrender.com' in origin:
...
def admin_process_deposit(deposit_id):
    ...
    try:
        ...
        print(f"📝 Deposit: Amount=${deposit.get('amount')}, Status={deposit.get('status')}")
        ...
        if action == 'approve':
            ...
            old_balance = user.get('wallet', {}).get('balance', 0)
...
# ==================== ADMIN - BULK RESEND DEPOSIT EMAILS ====================
...
def admin_process_investment(investment_id):
    ...
    try:
        ...
        if action == 'approve':
            ...
            old_total_invested = user.get('wallet', {}).get('total_invested', 0)
            new_total_invested = old_total_invested + investment['amount']
            ...
            if veloxtrades_users is not None:
                try:
                    ...
                    print(f"✅ Updated total_invested in veloxtrades_users")
                except Exception as e:
                    print(f"Error updating veloxtrades_users: {e}")
            ...
            if investment_users is not None:
                try:
                    ...
                    print(f"✅ Updated total_invested in investment_users")
                except Exception as e:
                    print(f"Error updating investment_users: {e}")
            ...
            try:
                create_notification(
                    user_id,
                    'Investment Approved! 🎉',
                    f'Your investment of ${investment["amount"]:,.2f} in {investment["plan_name"]} has been approved! Expected profit: ${investment["expected_profit"]:,.2f}',
                    'success'
                )
                print(f"✅ Notification created")
            except Exception as e:
...
@require_admin
def admin_manual_transaction():
    """Admin can manually add/remove balance, add referral bonus, complete investment, etc."""
    if request.method == "OPTIONS":
        response = make_response()
        response.headers['Access-Control-Allow-Origin'] = 'https://www.veloxtrades.com.ng'
        response.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization'
    ...
    try:
        ...
        amount = float(data.get('amount', 0))
        transaction_type = data.get('type', 'adjustment')  # adjustment, referral_bonus, investment_completion, refund
        ...
        user = None
        if veloxtrades_users is not None:
            user = veloxtrades_users.find_one({'username': username})
        
        if user is None and investment_users is not None:
            user = investment_users.find_one({'username': username})
        ...
        print(f"🔵 Admin manual transaction for {username}: ${amount} ({transaction_type})")
        
        # Update balance based on transaction type
        if transaction_type in ['adjustment', 'referral_bonus', 'investment_completion', 'refund']:
            # Add to balance
            if veloxtrades_users is not None:
                veloxtrades_users.update_one(
                    {'_id': ObjectId(user_id)},
                ...
                print(f"✅ Updated veloxtrades_users balance by +${amount}")
            ...
            if investment_users is not None:
                ...
                print(f"✅ Updated investment_users balance by +${amount}")
            
            # Update specific stats based on type
            if transaction_type == 'referral_bonus':
                if veloxtrades_users is not None:
                    veloxtrades_users.update_one(
                        {'_id': ObjectId(user_id)},
                ...
                description = f'Referral bonus: {description}'
                
            elif transaction_type == 'investment_completion':
                if veloxtrades_users is not None:
                    veloxtrades_users.update_one(
                        {'_id': ObjectId(user_id)},
                        {'$inc': {'wallet.total_profit': amount}}
                ...
                description = f'Investment profit: {description}'
        
        # Create transaction record
        transaction_id = 'MAN-' + ''.join(random.choices(string.digits + string.ascii_uppercase, k=12))
        ...
        if transactions_collection is not None:
            try:
                transactions_collection.insert_one({
                    ...
                    'admin_created': True,
                    'admin_id': str(get_user_from_request()['_id']),
                    'created_at': datetime.now(timezone.utc)
                })
        ...
        try:
            create_notification(
                user_id,
                f'{type_emoji} Balance Updated!',
                f'${amount:,.2f} has been added to your balance. Reason: {description}',
                'success'
            )
            print(f"✅ Notification created")
        except Exception as e:
...
A balance update has been made to your account!
...
Veloxtrades Team"""
        ...
        print(f"🔥 Manual transaction error: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': str(e)}), 500
...
def admin_process_withdrawal(withdrawal_id):
    ...
    try:
        ...
        action = data.get('action')
        reason = data.get('reason', '')
        transaction_id = data.get('transaction_id', '')  # For approve - blockchain tx hash
        
        print(f"🔵 Processing withdrawal {withdrawal_id}, action: {action}")
        
        # Find withdrawal - search in both databases
        withdrawal = None
        withdrawal_collection_used = None
        
        if veloxtrades_withdrawals is not None:
            try:
                withdrawal = veloxtrades_withdrawals.find_one({'_id': ObjectId(withdrawal_id)})
                if withdrawal:
                    withdrawal_collection_used = veloxtrades_withdrawals
                    print(f"✅ Found withdrawal in veloxtrades_withdrawals")
            except Exception as e:
                print(f"Error: {e}")
        
        if withdrawal is None and investment_withdrawals is not None:
            try:
                withdrawal = investment_withdrawals.find_one({'_id': ObjectId(withdrawal_id)})
                if withdrawal:
                    withdrawal_collection_used = investment_withdrawals
                    print(f"✅ Found withdrawal in investment_withdrawals")
            except Exception as e:
                print(f"Error: {e}")
        ...
        print(f"📝 Withdrawal: Amount=${withdrawal.get('amount')}, Status={withdrawal.get('status')}")
        ...
        if action == 'approve':
            ...
            if withdrawal_collection_used is not None:
                try:
                    withdrawal_collection_used.update_one(
                    ...
                    print(f"✅ Withdrawal status updated to approved")
                except Exception as e:
            ...
            try:
                create_notification(
                    withdrawal['user_id'],
                    'Withdrawal Approved! ✅',
                    f'Your withdrawal of ${withdrawal["amount"]:,.2f} has been approved and sent!',
                    'success'
                )
                print(f"✅ Notification created")
            except Exception as e:
        ...
        # ==================== REJECT ACTION ====================
            ...
            if withdrawal_collection_used is not None:
                try:
                    ...
                    print(f"✅ Withdrawal status updated to rejected")
                except Exception as e:
                    print(f"Error updating withdrawal: {e}")
            ...
            if veloxtrades_users is not None:
                try:
                    ...
                    print(f"✅ Refunded ${withdrawal['amount']} to user in veloxtrades_users")
                except Exception as e:
            ...
            if investment_users is not None:
                try:
                    ...
                    print(f"✅ Refunded ${withdrawal['amount']} to user in investment_users")
                except Exception as e:
                    print(f"Error refunding: {e}")
            ...
            try:
                create_notification(
                    withdrawal['user_id'],
                    'Withdrawal Rejected ❌',
                    f'Your withdrawal of ${withdrawal["amount"]:,.2f} was rejected. Reason: {reason}. Amount refunded.',
                    'error'
                )
                print(f"✅ Notification created")
            except Exception as e:
